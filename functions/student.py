...
def get_student(target_file_path: pathlib.Path, image_path: str, name_only: bool):
    ...
    for student_jp in data_jp_sorted:
        ...
        student_object_old = next(
            (x for x in data_outdated if x["id"] == student_id), {}
        )
        ...
        student_latest = Student(
            ...
            familyName=StudentName(
                ...
                en=next(
                    (x["familyName"] for x in data_en if x["id"] == student_id),
                    student_family_name_jp,
                ),
                kr=next(
                    (x["familyName"] for x in data_kr if x["id"] == student_id),
                    student_family_name_jp,
                ),
                tw=next(
                    (x["familyName"] for x in data_tw if x["id"] == student_id),
                    student_family_name_jp,
                ),
                th=next(
                    (x["familyName"] for x in data_th if x["id"] == student_id),
                    student_family_name_jp,
                ),
            ),
            name=StudentName(
                ...
                jp=student_name_jp,
                en=next(
                    (x["name"] for x in data_en if x["id"] == student_id),
                    student_name_jp,
                ),
                kr=next(
                    (x["name"] for x in data_kr if x["id"] == student_id),
                    student_name_jp,
                ),
                tw=next(
                    (x["name"] for x in data_tw if x["id"] == student_id),
                    student_name_jp,
                ),
                th=next(
                    (x["name"] for x in data_th if x["id"] == student_id),
                    student_name_jp,
                ),
            ),
            nickname=student_object_old["nickname"] if student_object_old else [],
            nicknameFrom=(
                student_object_old["nicknameFrom"]
                if student_object_old.get("nicknameFrom")
                else 0
            ),
            birthday=get_student_birthday(student_jp["birthday"]),
            club=find_student_club_name(student_jp["club"]),
            clubCode=student_jp["club"],
            affiliation=find_student_school_name(student_jp["affiliation"]),
            schoolCode=student_jp["affiliation"],
        ...
        if student_data_latest["nicknameFrom"] == 0:
            student_data_latest.pop("nicknameFrom")

        data_latest.append(student_data_latest)

    with open(target_file_path, "w", encoding="utf-8") as f:
        yaml.dump(
            data_latest, f, allow_unicode=True, sort_keys=False, Dumper=yaml.CDumper
        )

    image_output_path = (
        target_file_path.parent / "image" / "avatar_students"
        if image_path is None
        else pathlib.Path(image_path)
    )
